// THE WALL
// Demo of simple, conflict free portals
// 1. The wall (plus visible portal).
// Just a wall with a portal leading back to the same room
// Version with visible portal.
...
void DrawScene(mat4 worldToView) // Really draw a cell but we only have one here
{
	mat4 m;
...
void display(void)
{
	...
	DrawScene(worldToView * inverse(p->trans) * p->rot); // Draw neighbor cell - same cell rotated
	DrawScene(worldToView); // Draw current cell
